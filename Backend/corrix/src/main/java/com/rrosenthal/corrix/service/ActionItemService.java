...
public class ActionItemService {
    ...
    private void mapRequestToEntity(ActionItemRequest actionItemRequest, ActionItem actionItem) {
        ...
        actionItem.setStatus(statusRepository.findById(actionItemRequest.getStatusId()).orElseThrow(()-> new ResourceNotFoundException("Status not found")));

        if (actionItemRequest.getAssigneeId() != null) {
            User assignee = userRepository.findById(actionItemRequest.getAssigneeId()).orElseThrow(()-> new ResourceNotFoundException("User not found"));
